...
class OrdersController {
  ...
  async updateStatus(req, res) { 
    try { 
      console.log(`[OrdersController] Updating status for order ${req.params.id} to ${req.body.status}`);
      const order = await ordersService.updateStatus(req.params.id, req.body.status); 
      return sendSuccess(res, "Status updated", order); 
    } catch (error) { 
      console.error("[OrdersController] UpdateStatus Error:", error);
      return sendError(res, error.message + (error.stack ? " " + error.stack.split('\n')[1] : ""), 400); 
    } 
  }
